...
function Room03() {
  ...
  useEffect(() => {
    ...
    Axios.get(`http://localhost:${PORT}/bookingcontrolroom03`)
      .then(response => {
  ...
  const bookingStatus = (id, newStatus) => {
    Axios.put(`http://localhost:${PORT}/bookingcontrolroom03/update`, { status: 'Booking', id: id })
      .then((response) => {
  ...
  const cancelStatus = (id, newStatus) => {
    Axios.put(`http://localhost:${PORT}/bookingcontrolroom03/update`, { status: 'Available', id: id })
    .then((response) => {
      // If the status is updated successfully on the server, update the state
      setData(
        data.map((val) => {
